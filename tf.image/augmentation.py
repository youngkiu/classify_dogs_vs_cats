...
def random_image_scale(image,
  ...
  with tf.name_scope('RandomImageScale', values=[image]):
    ...
    size_coef = tf.random_uniform(
        [], minval=min_scale_ratio, maxval=max_scale_ratio, dtype=tf.float32, seed=seed)
...
def random_pixel_value_scale(image,
  ...
  with tf.name_scope('RandomPixelValueScale', values=[image]):
    color_coef = tf.random_uniform(
        tf.shape(image), minval=minval, maxval=maxval, dtype=tf.float32, seed=seed)
...
if __name__ == "__main__":
  ...
  # draw original image
  test_image = np.asarray(Image.open('test_chartreux.jpg'), dtype=np.float32)
  drawImage('original_image', test_image)
  ...
  with tf.Session() as sess:
    ...
    drawImage('random_horizontal_flip_image', random_horizontal_flip_image)
    drawImage('random_vertical_flip_image', random_vertical_flip_image)
    drawImage('random_rotation90_image', random_rotation90_image)
    drawImage('random_image_scale_image', random_image_scale_image)
    drawImage('random_pad_image', _random_pad_image)
    drawImage('random_crop_to_aspect_ratio_image',
              random_crop_to_aspect_ratio_image)
    drawImage('random_pad_to_aspect_ratio_image',
              random_pad_to_aspect_ratio_image)
    ...
    drawImage('random_pixel_value_scale_image', random_pixel_value_scale_image)
    drawImage('random_rgb_to_gray_image', random_rgb_to_gray_image)
    drawImage('random_adjust_brightness_image', random_adjust_brightness_image)
    drawImage('random_adjust_contrast_image', random_adjust_contrast_image)
    drawImage('random_adjust_hue_image', random_adjust_hue_image)
    drawImage('random_adjust_saturation_image', random_adjust_saturation_image)
    drawImage('random_distort_color_image', random_distort_color_image)

    # miscellaneous augmentation
    random_black_patches_image = sess.run(
        random_black_patches(X), feed_dict={X: test_image})
    drawImage('random_black_patches_image', random_black_patches_image)
